...
class Perfil(models.Model):
    ...
    def recalcular_calificacion_promedio(self):
        """
        Recalcula la calificación promedio basándose en las reseñas activas.
        """
        nueva_calificacion = self.resenas.filter(activa=True).aggregate(
            promedio=Avg('puntuacion')
        )['promedio']

        if nueva_calificacion is not None:
            self.calificacion_promedio = round(nueva_calificacion, 2)
        else:
            self.calificacion_promedio = 0.00
...
def actualizar_calificacion_al_guardar(sender, instance, **kwargs):
    """
    Actualiza la calificación del perfil del estudiante cuando se guarda una reseña.
    """
    instance.estudiante.recalcular_calificacion_promedio()

@receiver(post_delete, sender=Resena)
def actualizar_calificacion_al_eliminar(sender, instance, **kwargs):
    """
    Actualiza la calificación del perfil del estudiante cuando se elimina una reseña.
    """
    instance.estudiante.recalcular_calificacion_promedio()
